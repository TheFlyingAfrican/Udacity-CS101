...
# 23 = 16 + 4 + 2 + 1 which means that 'x..', '.x.', '..x' an d '...' all 
...
# pattern determines the factors that goes into a number
def patternFactor(n):
    factors = []
    power = 7
    while power > 0:
...
#takes in a pattern number and returns a dictionary with the ruleset
def createRuleSet(factors):
    patternSet = {1:    ['...','.'],
                  2:    ['..x','.'],
                  4:    ['.x.','.'],
                  8:    ['.xx','.'],
    ...
    for f in factors:
        patternSet[f][1] = 'x'
    for entry in patternSet:
        ruleSet[patternSet[entry][0]] = patternSet[entry][1]
    return ruleSet

def cellular_automaton(initialState, patternNo, generation):
    if initialState == '':
        return None

    patternSet = createRuleSet(patternFactor(patternNo))
    lookupPattern = ''
    data = initialState[len(initialState)-1] + initialState + initialState[0]
    nextRow = ''
   
    for counter in range(generation):
        for i in range(len(initialState)):
            lookupPattern = data[i] + data[i+1] + data[i+2]
            nextRow += patternSet[lookupPattern]
        data = nextRow
        data = data[len(data)-1] + data + data[0]
        nextRow = ''
