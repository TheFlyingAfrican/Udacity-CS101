...
def date_converter(translate_table, dateString):
    month = dateString[0:dateString.find('/')]
    year = dateString[dateString.find('/',dateString.find('/')+1)+1:]
    day = dateString[dateString.find('/')+1:dateString.find('/',dateString.find('/')+1)]
    return (str(day) + ' ' + translate_table[int(month)] + ' ' + str(year))
