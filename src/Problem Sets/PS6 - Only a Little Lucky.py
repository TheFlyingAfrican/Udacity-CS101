...
def quick_sort(list, start, end):
    ...
    if (end - start) >= 1:
        ...
        while (k > i):
            while (list[i][0] <= pivot) and (i <= end) and (k >i):
                i += 1
            while (list[k][0] > pivot) and (k >= start) and (k >= i):
                k -= 1
            if (k > i):
                swap(list, i, k)
        swap(list, start, k)
        quick_sort(list, start, k-1)
        quick_sort(list, k+1, end)
    return list
...
def ordered_search(index, ranks, keyword):
    if keyword in index:
        ...
        for url in index[keyword]:
            url_rank_list.append([ranks[url],url])
        quick_sort(url_rank_list,0,len(url_rank_list)-1)
        
        returnlist = []
        for i in range(0,len(url_rank_list)):
            returnlist.append(url_rank_list.pop()[1])
        return returnlist
