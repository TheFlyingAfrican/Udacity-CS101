...
################################################################
# to test lists within lists within lists
################################################################
# index = [
#          ['keyword', [['url',1], ['url2',1]]], 
#          ['keyword2', [['url3',1]]]
#          ]
...
# index[0] = ['keyword', [['url',1], ['url2',1]]]
# index[0][0] = keyword
# index[0][1][0] = ['url',1]
# index[0][1][0][0] = url
# index[0][1][0][1] = 1
# index[0][1].append(['url4',3])
# for i in index:
#     print i
#     print i[1]
#     for j in i[1]:
...
def add_to_index(index, keyword, url):
    for entry in index:
        if entry[0] == keyword:
            for subindex in entry[1]:
            #entry[1] is the list of urls and count i.e. [[url,count],[url2,count2]], subindex is the list element [url,count]. 
                print entry
                print entry[0]
                print entry[1]
                print subindex
                ...
                if url not in subindex[0]:
                    entry[1].append([url,0])
                    #subindex.append([url, 0]) #this is the bug, you're at the wrong level! you're appending to the element within the list instead of the list itself
                    return
...
def lookup(index, keyword): #return a list of all urls with specified keyword from the index
    for entry in index:
        if entry[0] == keyword:
            return entry[1]
    return None
